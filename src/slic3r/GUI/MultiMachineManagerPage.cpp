    ...
    : DeviceItem(parent, obj)
{
    ...
    Bind(EVT_MULTI_DEVICE_VIEW, [this, obj](auto& e) {
        ...
        StatusPanel* sp = nullptr;
        if (wxGetApp().mainframe->m_openbambu_monitor)
            sp = wxGetApp().mainframe->m_openbambu_monitor->get_status_panel();
        else if (wxGetApp().mainframe->m_monitor)
            sp = wxGetApp().mainframe->m_monitor->get_status_panel();
        if (sp && sp->get_media_play_ctrl())
            sp->get_media_play_ctrl()->jump_to_play();
    });
